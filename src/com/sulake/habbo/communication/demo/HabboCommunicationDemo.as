package com.sulake.habbo.communication.demo
{
   ...
   public class HabboCommunicationDemo extends Component
   {
      ...
      private var var_553:IKeyExchange;
      
      private var var_1028:String;
      
      private var _view:HabboLoginDemoView;
      
      private var var_2752:PseudoRandom;
      ...
      private const const_1863:String = "fuselogin";
      ...
      public function setSSOTicket(param1:String) : void
      {
         ...
         {
            this.var_673 = param1;
            this.var_56.initConnection(HabboConnectionType.const_281);
         }
      }
      ...
      {
         if(this.var_56 && this.var_140 && this.var_16 && this._windowManager)
         {
      ...
      private function componentsAreRunning() : void
      {
         ...
         var _loc1_:Boolean = this.var_140.getBoolean("client.hotel_view.show_on_startup",false);
         this.var_235 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         var _loc2_:String = this.var_140.getKey("client.hotel_view.image");
         var _loc3_:String = this.var_140.getKey("image.library.url");
         if(_loc2_ != null && _loc3_ != null)
         {
            this.var_235.loadHotelViewImage(_loc3_ + _loc2_);
         }
         var _loc4_:* = this.var_140.getKey("use.sso.ticket") == "1";
         this.var_673 = this.var_140.getKey("sso.ticket",null);
         this.var_2002 = this.var_140.getKey("flash.client.url","");
         this.var_2753 = this.var_140.getKey("external.variables.txt","");
         if(this.var_235 == null)
         {
            this.var_235 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         }
         if(_loc4_)
         {
            this.var_56.mode = HabboConnectionType.const_98;
            if(this.var_673)
            {
               this.var_56.initConnection(HabboConnectionType.const_281);
            }
         ...
         else if(this._windowManager != null)
         {
            ...
            if(_loc5_)
            {
               ...
               this._view.addEventListener(HabboLoginDemoView.const_495,this.onInitConnection);
            }
         }
      }
      ...
      private function onInitConnection(param1:Event = null) : void
      {
         ...
         {
            this.var_56.mode = HabboConnectionType.const_98;
            if(this.var_140.keyExists("local.environment"))
            {
               this.var_56.mode = HabboConnectionType.const_1094;
            }
         }
         this.var_56.initConnection(HabboConnectionType.const_281);
      }
      ...
      private function onConnectionEstablished(param1:Event = null) : void
      {
         ...
         if(_loc2_ != null)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_972);
      ...
      private function onInitCrypto(param1:IMessageEvent) : void
      {
         ...
         this.var_2752 = new PseudoRandom(parseInt(_loc4_.substring(_loc4_.length - 4),16),65536);
         switch(this.var_56.mode)
         {
            case HabboConnectionType.const_98:
               _loc6_ = "";
               _loc6_ = this.var_140.getKey("url.prefix",_loc6_);
               _loc6_ = _loc6_.replace("http://","");
               _loc6_ = _loc6_.replace("https://","");
               _loc7_ = this.var_140.getKey("hotelview.banner.url","http:/\nsitename$/gamedata/banner");
               _loc7_ = _loc7_.replace("$sitename$",_loc6_);
               this.var_1544 = _loc4_;
               this.var_235.loadBannerImage(_loc7_ + "?token=" + this.var_1544,this.onHotelViewBannerLoaded);
               break;
            case HabboConnectionType.const_1094:
               this.sendConnectionParameters(_loc2_);
               break;
            case HabboConnectionType.const_1590:
               this.var_1028 = this.generateRandomHexString(30);
               this.var_553 = new DiffieHellman(new BigInteger(this.var_140.getKey("connection.development.prime"),16),new BigInteger(this.var_140.getKey("connection.development.generator"),16));
               this.var_553.init(this.var_1028);
               _loc8_ = this.var_553.method_7(10);
      ...
      private function onSecretKeyEvent(param1:IMessageEvent) : void
      {
         ...
         this.var_553.generateSharedKey(_loc4_,10);
         var _loc5_:String = this.var_553.getSharedKey(16).toUpperCase();
         var _loc6_:RC4_R27 = new RC4_R27(null,null);
         var _loc7_:IEncryption = new RC4_R27(_loc6_,this.var_2752);
      ...
      private function sendConnectionParameters(param1:IConnection) : void
      {
         ...
         connection.send(new VersionCheckMessageComposer(401,this.var_2002,this.var_2753));
         var machineId:String = "";
         try
         {
            so = SharedObject.getLocal(this.const_1863,"/");
      ...
      private function onSessionParamsEvent(param1:IMessageEvent) : void
      {
         ...
         if(this.var_673 != null)
         {
            _loc4_ = new SSOTicketMessageComposer(this.var_673);
            _loc2_.send(_loc4_);
      ...
      private function onConnectionDisconnected(param1:Event) : void
      {
         ...
         if(param1.type == Event.CLOSE && !this.var_2751)
         {
            _loc2_ = this.var_140.getKey("logout.disconnect.url");
      ...
      private function onDisconnectReason(param1:DisconnectReasonEvent) : void
      {
         ...
         this.var_2751 = true;
         var _loc2_:String = this.var_140.getKey("logout.url");
         if(_loc2_.length > 0)
         {
      ...
      private function setOriginProperty(param1:String) : String
      {
         ...
         {
            return param1.replace("%origin%",this.var_140.getKey("flash.client.origin","popup"));
         }
         return param1;
      }
      ...
      private function onHotelViewBannerLoaded(param1:BitmapData) : void
      {
         ...
         var _loc4_:String = xor(_loc3_,this.var_1544);
         ...
         this.var_553 = new DiffieHellman(_loc10_,_loc11_);
         var _loc13_:int = 10;
         var _loc14_:* = null;
         while(_loc13_ > 0)
         {
            _loc14_ = this.generateRandomHexString(30);
            this.var_553.init(_loc14_);
            _loc15_ = this.var_553.method_7(10);
            if(_loc15_.length >= 64)
            {
               _loc12_ = _loc15_;
               this.var_1028 = _loc14_;
            }
