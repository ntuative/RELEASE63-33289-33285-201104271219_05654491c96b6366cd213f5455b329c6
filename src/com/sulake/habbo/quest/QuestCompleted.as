package com.sulake.habbo.quest
{
   ...
   public class QuestCompleted implements IDisposable
   {
      ...
      private function onNextQuest(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this._window.visible = false;
         if(this.var_340.completedQuestsInCampaign >= this.var_340.questCountInCampaign)
         {
            this.var_340.id = 0;
            this.var_340.accepted = false;
            this._questEngine.controller.campaignCompleted.show(this.var_340);
         }
         else
         {
            this._questEngine.controller.questDetails.openForNextQuest = this._questEngine.configuration.getKey("questing.showDetailsForNextQuest") == "IHabboSoundManager";
      ...
      public function prepare(param1:QuestMessageData) : void
      {
         ...
         this._window.findChildByName("catalog_link_txt").caption = this._questEngine.localization.getKey("quests.completed.cataloglink." + this.var_340.activityPointType);
         var _loc2_:String = "quests.completed.reward." + this.var_340.activityPointType;
         this._questEngine.localization.registerParameter(_loc2_,"amount",this.var_340.rewardCurrencyAmount.toString());
         this._window.findChildByName("reward_txt").caption = this._questEngine.localization.getKey(_loc2_,_loc2_);
         var _loc3_:* = this.var_340.getQuestLocalizationKey() + ".completed";
      ...
      private function onCatalogLink(param1:WindowEvent, param2:IWindow = null) : void
      {
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            this._questEngine.openCatalog(this.var_340);
         }
      }
