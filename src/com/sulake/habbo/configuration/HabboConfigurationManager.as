package com.sulake.habbo.configuration
{
   ...
   public class HabboConfigurationManager extends Component implements IHabboConfigurationManager
   {
      ...
      private static var var_1362:String = "%";
      ...
      public function HabboConfigurationManager(param1:IContext, param2:uint = 0, param3:IAssetLibrary = null)
      {
         ...
         this.var_582 = this.getKey("url_prefix");
         if(this.var_582 != null && this.var_582.length > 0)
         {
            this.setKey("client.fatal.error.url",this.var_582 + "/flash_client_error");
         }
         this.init();
      }
      ...
      private function onConfigurationError(param1:Event = null) : void
      {
         ...
         Core.error("Could not load external variables. URL " + this.getKey("external.variables.txt") + "HTTP status " + _loc3_ + ". Client startup failed!",true,Core.const_1737);
      }
      ...
      private function parseConfiguration(param1:String) : void
      {
         ...
         for each(line in rows)
         {
            if(line.substr(0,1) != "#")
            {
               ...
               if(pair[0].length > 0 && pair[1].length > 0)
               {
                  ...
                  if(key == "configuration.readonly" && value == "IHabboSoundManager")
                  {
                     readOnly = true;
                  }
         ...
         if(this.getKey("local.environment") == "IHabboSoundManager")
         {
            this.setKey("url.prefix",this.getKey("url.prefix.local"));
         }
         this.var_582 = this.getKey("url.prefix");
         if(this.var_582.length > 0)
         {
            externalAccessDomain = this.var_582.replace("http://","");
      ...
      public function getKey(param1:String, param2:String = "", param3:Dictionary = null) : String
      {
         ...
         if(_loc4_ != null)
         {
            ...
            {
               _loc4_ = _loc4_.replace("%predefined%",this.var_582);
      ...
      public function setKey(param1:String, param2:String, param3:Boolean = false, param4:Boolean = false) : void
      {
         ...
         if(this.var_1637.indexOf(param1) < 0 || param3)
         {
      ...
      private function fillParams(param1:String, param2:Dictionary) : String
      {
         ...
         while(_loc3_ < 10)
         {
            _loc4_ = param1.indexOf(var_1362);
            if(_loc4_ < 0)
            {
               break;
            }
            _loc5_ = param1.indexOf(var_1362,_loc4_ + 1);
            if(_loc5_ < 0)
            {
               break;
            }
      ...
      public function getBoolean(param1:String, param2:Boolean) : Boolean
      {
         ...
         return _loc3_ == "1" || _loc3_.toLowerCase() == "IHabboSoundManager";
      }
   }
}
