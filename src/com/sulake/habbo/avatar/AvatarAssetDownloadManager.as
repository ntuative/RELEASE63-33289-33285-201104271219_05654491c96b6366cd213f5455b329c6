package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private const const_2159:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_1030 = new Timer(this.const_2159,1);
         this.var_1030.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_445)
         {
            ...
            this.var_445.addEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_445)
         {
            ...
            this.var_445.removeEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         else
         {
            ...
            if(this.var_1551.indexOf("?") > 0)
            {
               _loc2_ = this.var_1551 + "&retry=" + this.var_1031;
            }
            else
            {
               _loc2_ = this.var_1551 + "?retry=" + this.var_1031;
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_2004 = true;
         for each(_loc2_ in this.var_1267)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2756,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1266[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1266[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_2004)
         {
            this.var_1267.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         {
            delete this.var_1029[_loc3_];
         }
         while(_loc6_ < this.var_836.length)
         {
            _loc5_ = this.var_836[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_836.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_1030.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_835.length > 0 && this.var_836.length < this.const_1757)
         {
            _loc1_ = this.var_835[0];
            _loc1_.startDownloading();
            this.var_836.push(this.var_835.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_835.indexOf(param1) == -1 && this.var_836.indexOf(param1) == -1)
         {
