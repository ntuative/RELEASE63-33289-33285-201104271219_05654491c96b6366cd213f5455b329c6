package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private function createView() : void
      {
         ...
         this.var_351 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_613 && this.var_1434)
         {
            _loc5_ = this._controller.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_96,this.var_945,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_37,parseInt(FigureData.const_905));
         ...
         if(this.var_351)
         {
            if(this.var_351.bitmap)
            {
               this.var_351.bitmap.dispose();
            }
            this.var_351.bitmap = new BitmapData(this.var_351.width,this.var_351.height,true,0);
            _loc7_ = (this.var_351.width - _loc1_.width) / 2;
            _loc8_ = (this.var_351.height - _loc1_.height) / 2;
            this.var_351.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               this.var_613 = this._controller.figureData.getFigureString();
               this.var_945 = this._controller.gender;
               this._controller.handler.saveWardrobeOutfit(this.var_1771,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_613)
               {
                  this._controller.loadAvatarInEditor(this.var_613,this.var_945,this._controller.clubMemberLevel);
               }
         }
      }
