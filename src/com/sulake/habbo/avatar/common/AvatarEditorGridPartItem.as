package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private const const_2142:int = 13421772;
      
      private const const_2141:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1962 = Math.max(this.var_1962,_loc6_.colorLayerIndex);
            }
         }
         this.var_440 = this.var_66.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_699 != null && !this.var_1961)
            {
               _loc3_ = this.var_699;
            }
            else
            ...
            if(this.var_825)
            {
               switch(this.var_825.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_44:
                     _loc2_.style = HabboIconType.const_831;
                     break;
                  case HabboClubLevelEnum.const_46:
                     _loc2_.style = HabboIconType.const_906;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1525.color = this.const_2142;
         }
         else
         {
            this.var_1525.color = this.const_2141;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_366[_loc6_] + "_" + FigureData.const_146;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_661 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1960)
               {
                  ...
                  this.var_1960 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_661.width,this.var_661.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_661.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_661.y;
               if(this.var_1961 && _loc2_.colorLayerIndex > 0)
               {
