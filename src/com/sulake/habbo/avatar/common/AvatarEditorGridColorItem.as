package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridColorItem
   {
      ...
      private var var_1036:IPartColor;
      ...
      private function updateThumbData() : void
      {
         ...
         if(_loc2_ != null)
         {
            _loc2_.bitmap = new BitmapData(this.var_934.width,this.var_934.height,true,0);
            _loc5_ = this.var_934.clone();
            _loc5_.colorTransform(_loc5_.rect,this.var_1036.colorTransform);
         ...
         if(_loc3_ != null)
         {
            if(this._isSelected)
            {
               _loc3_.bitmap = new BitmapData(this.var_936.width,this.var_936.height,true,16777215);
               _loc3_.bitmap.copyPixels(this.var_936,this.var_936.rect,new Point(0,0),null,null,true);
               _loc3_.visible = true;
            }
            else
            {
         ...
         if(_loc4_)
         {
            if(this.var_1036)
            {
               switch(this.var_1036.clubLevel)
               {
                  case HabboClubLevelEnum.const_54:
                     _loc4_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_44:
                     _loc4_.style = HabboIconType.const_831;
                     _loc4_.visible = true;
                     break;
                  case HabboClubLevelEnum.const_46:
                     _loc4_.style = HabboIconType.const_906;
                     _loc4_.visible = true;
               }
            }
            else
