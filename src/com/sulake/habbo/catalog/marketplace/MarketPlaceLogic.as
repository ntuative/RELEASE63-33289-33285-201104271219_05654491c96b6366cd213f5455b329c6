package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      public const const_2075:int = 1;
      
      public const const_1981:int = 2;
      
      public const const_2173:int = 3;
      ...
      private var var_2830:MarketplaceItemStats;
      ...
      public function dispose() : void
      {
         ...
         {
            this.disposeOffers(this.var_278);
            this.var_278 = null;
         }
      ...
      {
         this.requestOffers(this.var_2833,this.var_2832,this.var_2678,this.var_2831);
      }
      ...
      {
         if(!this.var_278 || !this._catalog || !this._catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = this.var_278.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         this.showConfirmation(this.const_2075,_loc2_);
      }
      ...
      public function redeemSoldOffers() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            ...
            if(_loc3_.status == MarketPlaceOfferState.const_494)
            {
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            this.var_278.add(_loc4_.offerId,_loc5_);
         }
         if(this._visualization != null)
         {
            this._visualization.listUpdatedNotify();
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         this.var_2693 = _loc3_.creditsWaiting;
         for each(_loc4_ in _loc3_.offers)
         {
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            item = this.var_278.remove(parser.requestedOfferId);
            if(item != null)
            {
               item.dispose();
            }
         ...
         {
            updateItem = this.var_278.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               this.var_278.add(parser.offerId,updateItem);
            }
            this.var_278.remove(parser.requestedOfferId);
            this.showConfirmation(this.const_1981,updateItem);
            if(this._visualization != null)
            {
               this._visualization.listUpdatedNotify();
            }
      ...
      {
         if(param1.furniCategoryId != this.var_2650 || param1.furniTypeId != this.var_2649)
         {
            return;
         }
